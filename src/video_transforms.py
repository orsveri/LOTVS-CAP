...
from src.rand_augment import rand_augment_transform, rand_augment_double_transform
from src.random_erasing import RandomErasing
from src import functional as FF
...
def create_random_double_augment(
        input_size,
        auto_augment=None,
        interpolation="bilinear",
        do_transforms=None,
        ok_geometric_transforms=None,
):
    """
    Get video randaug transform.
    ...
    if auto_augment and auto_augment.startswith("rand"):
        assert isinstance(auto_augment, str)
        if isinstance(img_size, tuple):
            img_size_min = min(img_size)
        else:
        ...
        aa_params = {"translate_const": int(img_size_min * 0.45)}
        return transforms.Compose(
            [rand_augment_double_transform(
                auto_augment, aa_params, do_transforms, ok_geometric_transforms=ok_geometric_transforms
            )]
        )
    raise NotImplementedError
